...
use app\knowledge\validate\ArticleCheck;
    ...
    public function index()
    {
        if (request()->isAjax()) {
            ...
            if (!empty($param['article_cate_id'])) {
                $where[] = ['a.article_cate_id', '=', $param['Knowledge_cate_id']];
            }
            $where[] = ['a.status', '>=', 0];
            $where[] = ['a.is_share', '=', 1];
    ...
    public function add()
    {
        ...
        if (request()->isAjax()) {
            ...
            if (!empty($param['id']) && $param['id'] > 0) {
                ...
                Db::startTrans();
                try {
                    $res = KnowledgeList::strict(false)->field(true)->update($param);
                    $aid = $param['id'];
                    if ($res) {
                        //关联关键字
                        if (isset($param['keyword_names']) && $param['keyword_names']) {
                            ...
                            $res_keyword = (new KnowledgeList())->insertKeyword($keywordArray, $aid);
                        } else {
                            $res_keyword == true;
                        }
                ...
                try {
                    ...
                    $aid = KnowledgeList::strict(false)->field(true)->insertGetId($param);
                    if ($aid) {
                        //关联关键字
                        if (isset($param['keyword_names']) && $param['keyword_names']) {
                            $keywordArray = explode(',', $param['keyword_names']);
            ...
            View::assign('id', $id);
            if ($id > 0) {
                $article = (new KnowledgeList())->detail($id);
